...
static void display_bad_wifi(void) {
    ssd1306_display_text(0, "Bad WiFi Creds!!", 16);
    ssd1306_display_text(1, "                ", 16);
    ssd1306_display_text(2, "Make sure WiFi's", 16);
    ssd1306_display_text(3, "correctly workin", 16);
    ssd1306_display_text(4, "                ", 16);
    ssd1306_display_text(5, "If so, pass the ", 16);
    ssd1306_display_text(6, "correct creds :)", 16);
...
static void verify_cert(mbedtls_net_context server_fd) {
    ...
    if (ret != 0 && ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
        ...
        if ((flags = (int) mbedtls_ssl_get_verify_result(&ssl)) != 0) {
