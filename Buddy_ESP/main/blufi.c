...
void blufi_dh_negotiate_data_handler(uint8_t *data, uint8_t **output_data, int *output_len, bool *need_free)
{
    int ret;
    uint8_t type = data[0];
    ...
    switch (type) {
        ...
        case SEC_TYPE_DH_PARAM_DATA:
            ...
            free(blufi_sec->dh_param);
            blufi_sec->dh_param = NULL;
            ret = mbedtls_dhm_make_public(&blufi_sec->dhm, (int) mbedtls_mpi_size( &blufi_sec->dhm.P ), blufi_sec->self_public_key, blufi_sec->dhm.len,
                                          (int (*)(void *, unsigned char *, size_t)) myrand, NULL);
            if (ret) {
            ...
            *output_len = (int)blufi_sec->dhm.len;
            *need_free = false;
            break;
        default:
            break;
...
uint16_t blufi_crc_checksum(uint8_t *data, int len)
{
