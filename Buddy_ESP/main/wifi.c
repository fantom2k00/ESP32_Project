...
static void ip_event_handler(int32_t event_id) {
    wifi_mode_t mode;
...
static void initialise_wifi(void)
{
    ...
    assert(sta_netif);
    ESP_ERROR_CHECK(esp_event_handler_register(WIFI_EVENT, ESP_EVENT_ANY_ID, (esp_event_handler_t) &wifi_event_handler, NULL));
    ESP_ERROR_CHECK(esp_event_handler_register(IP_EVENT, IP_EVENT_STA_GOT_IP, (esp_event_handler_t) &ip_event_handler, NULL));
...
static esp_blufi_callbacks_t callbacks = {
        ...
        .checksum_func = (esp_blufi_checksum_func_t) blufi_crc_checksum,
};
...
static void event_callback(esp_blufi_cb_event_t event, esp_blufi_cb_param_t *param)
{
    switch (event) {
        ...
        case ESP_BLUFI_EVENT_RECV_USERNAME:
            ESP_LOGD("BLUFI", "Received username");
            break;
        case ESP_BLUFI_EVENT_RECV_CA_CERT:
            ESP_LOGD("BLUFI", "Received CA Certificate");
            break;
        case ESP_BLUFI_EVENT_RECV_CLIENT_CERT:
            ESP_LOGD("BLUFI", "Received Client's Certificate");
            break;
        case ESP_BLUFI_EVENT_RECV_SERVER_CERT:
            ESP_LOGD("BLUFI", "Received Server's Certificate");
            break;
        case ESP_BLUFI_EVENT_RECV_CLIENT_PRIV_KEY:
            ESP_LOGD("BLUFI", "Received Client's Private Key");
            break;
        case ESP_BLUFI_EVENT_RECV_SERVER_PRIV_KEY:
            ESP_LOGD("BLUFI", "Received Client's Server Key");
            break;
        default:
            break;
    }
